...
function updateTotal() {
  ...
  for (var i = 0; i < cartCard.length; i++) {
    ...
    if (checkbox.checked) {
      ...
      document.getElementsByClassName("cart__prices-total")[0].innerText =
        "$" + totalItem;
      document.getElementsByClassName("cart__prices-item")[0].innerText =
        countItems + " items";
    }
  }
  if (countCheck == 0) {
...
function plusQuantity(event) {
  ...
  var input = btn.parentElement.parentElement.getElementsByClassName(
    "cart__amount-number"
  )[0];
...
function addCardtoCart(title, price, img, quantity) {
  ...
  for (var i = 0; i < cartItemsNames.length; i++) {
    if (title === cartItemsNames[i].innerHTML) {
      ...
      var quantity_cart = parseInt(
        item.getElementsByClassName("cart__amount-number")[0].value
      );
      item.getElementsByClassName("cart__amount-number")[0].value =
        quantity_cart + parseInt(quantity);
      return;
    }
  }
  ...
  cartShopBox
    ...
    .addEventListener("click", removeCartcard);
  cartShopBox
...
function choiceAddress(name, phone, addder) {
  ...
  btn_xn.addEventListener("click", function () {
    ...
    (phone_value.innerText = phone), (address_value.innerText = addder);
    closeDlAddress();
  });
}
function addAddress() {
  ...
  list.append(newAdd);
  newAdd
    .getElementsByClassName("btn__xoa")[0]
...
function showReview(event) {
  ...
  document
    .getElementsByClassName("btn__submit")[0]
...
function next(event) {
  ...
  if (
    two.className === "progress two active" &&
    three.className !== "progress three active" &&
    ...
    three.className === "progress three active" &&
    four.className !== "progress four active" &&
    five.className !== "progress four active"
  ) {
...
function staff_HD(event) {
  ...
  if (
    three.className === "progress three active" &&
    five.className !== "progress five active"
  ) {
...
for (var i = 0; i < card_dl.length; i++) {
  ...
  if (precent_dl.innerText !== "") {
    ...
    var price_noneDiscount_dl =
      card_dl[i].getElementsByClassName("noneDiscount")[0];
